  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-slate-900 hover:bg-black text-white shadow-2xl z-[100] flex items-center justify-center transition-all hover:scale-110 active:scale-90 border-2 border-red-500/20"
        >
          <Bot className="h-5 w-5 sm:h-6 sm:w-6 text-red-500" />
        </Button>
      </DialogTrigger>
        ...
        <div className="relative">
          ...
          <div className="p-8 space-y-6">
            ...
            <form onSubmit={handleSearch} className="relative group">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-red-500/20 to-pink-500/20 rounded-2xl blur-sm opacity-0 group-focus-within:opacity-100 transition duration-500"></div>
              <div className="relative">
                <input
                  type="text"
                  value={query}
                ...
                <button
                  ...
                  className="absolute right-2 top-1/2 -translate-y-1/2 bg-slate-900 dark:bg-rose-500 hover:bg-black dark:hover:bg-rose-600 disabled:bg-slate-200 dark:disabled:bg-slate-700/50 text-white disabled:text-slate-400 dark:disabled:text-slate-500 p-2.5 rounded-xl transition-all shadow-md active:scale-90"
                >
                  {isLoading ? (
                ...
                {isLoading ? (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="text-[10px] font-bold text-red-500 uppercase tracking-widest block mx-auto py-2"
                    >
                      Nova Pergunta
                    </button>
                  </motion.div>
